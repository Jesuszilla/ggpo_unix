...
CreateSocket(uint16_t bind_port, int retries)
{
   ...
   uint16_t port;
   int optval = 1;
   struct linger optval2 = { 0, 1 };

   s = socket(AF_INET, SOCK_DGRAM, 0);
   setsockopt(s, SOL_SOCKET, SO_REUSEADDR, (const void *)&optval, sizeof optval);
   setsockopt(s, SOL_SOCKET, SO_LINGER, (const void *)&optval2, sizeof optval2);
...
Udp::SendTo(char *buffer, int len, int flags, struct sockaddr *dst, int destlen)
{
   ...
   ssize_t res = sendto(_socket, buffer, len, flags, dst, destlen);
   if (res <= 0) {
      int err = errno;
      Log("unknown error in sendto (erro: %d  wsaerr: %d).\n", res, err);
      ASSERT(false && "Unknown error in sendto");
...
Udp::OnLoopPoll(void *cookie)
{
   ...
   for (;;) {
      ...
      if (len == -1) {
         int error = errno;
         if (error != ENOBUFS) {
            Log("recvfrom errno returned %d (%x).\n", error, error);
         }
         break;
      } else if (len > 0) {
         char src_ip[1024];
